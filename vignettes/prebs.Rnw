...
\textbf{paired\_ended\_reads}, \textbf{ignore\_strand}, \textbf{count\_multiple} & Options that control the process of counting reads
\end{longtable}
...
\calcprebs{} has a few arguments that control the process of the read counting. Firstly, \texttt{paired\_ended\_reads} ensures the correct treatment of paired-ended reads. If your data contains paired-ended reads, you should set this option to \texttt{TRUE}, otherwise the two mate reads will be treated as independent units. Secondly, \texttt{ignore\_strand} controls whether the strand from which the reads comes should be considered during read-counting. If your data comes from strand-specific RNA-seq protocol, set this option to FALSE, otherwise, leave it at its default value (TRUE). Finally, \texttt{count\_multiple} defines how the reads that overlap with several probe regions should be treated. If it is set to TRUE, then the read is counted for all overlapping probe regions, otherwise, it is counted only for one probe region (picked at random from all overlapping ones). We recommend to leave this option at its default value (FALSE).
